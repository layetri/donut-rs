...
pub struct Synth {
    ...
    pub effects: Vec<Box<dyn AudioEffect + Send + Sync>>,
    pub sample_rate: f32,
...
impl Synth {
    pub fn new(sample_rate: f32, block_size: usize) -> Self {
        ...
        Synth {
            voices,
            effects: vec![
                Box::new(Saturator::new(block_size, sample_rate))
            ],
            sample_rate,
            block_size,
            voices_in_use: 0,
    ...
    pub fn process(&mut self) -> Buffer {
        ...
        let note_offs = self.clock.get_note_offs().clone();
        ...
        let mut mix = self.mix.process(&outputs, self.block_size, self.voices_in_use as f32);

        for effect in self.effects.iter_mut() {
            effect.process(&mut mix);
        }
