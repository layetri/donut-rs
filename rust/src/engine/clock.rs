...
pub struct Clock {
    ...
    pub note_ons: SmallVec<[Note; 16]>,
    pub note_offs: SmallVec<[u8; 32]>,
...
impl Clock {
    ...
    pub fn tick(&mut self) -> Option<usize> {
        if self.is_playing {
            ...
            self.position = (self.sample_position as f32 / self.sample_rate * self.bpm / 60.0 * self.ppq as f32).floor() as usize;
    ...
    pub fn get_notes(&self) -> SmallVec<[Note; 16]> {
        self.note_ons.clone()
    }

    pub fn get_note_offs(&mut self) -> &SmallVec<[u8; 32]> {
