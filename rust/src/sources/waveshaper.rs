...
use crate::system::parameter::ParameterID::{WS1Amount, WS1Detune, WS1Harmonics};
...
impl WaveShaper {
    pub fn new(sample_rate: f32, buffer_size: usize, voice_id: usize) -> Self {
        let mut n: SmallVec<[f32; 16]> = smallvec![];
        for i in 0..16 {
        ...
        let mut level = Parameter::from_id(WS1Amount, module_id, voice_id, sample_rate);
...
impl AudioSource for WaveShaper {
    ...
    fn get_parameters_mut(&mut self) -> SmallVec<[&mut Parameter; 16]> {
        smallvec![&mut self.harmonics, &mut self.detune, &mut self.level]
    }
